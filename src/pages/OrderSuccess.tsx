...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    fetchBranches()
      .then((branches) => {
        if (!active) return
        // Huwara branch is the primary contact for confirmations.
        const huwara = branches.find(
          (b) => b.name_ar?.includes('حوارة') || b.name_en?.toLowerCase().includes('huwara'),
        )
        const num = huwara?.whatsapp || branches.find((b) => b.whatsapp)?.whatsapp
  ...
  // Direct visit with no order — neutral state, no crash.
  if (!orderNumber) {
    return (
